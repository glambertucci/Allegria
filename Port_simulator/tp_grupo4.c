...
//Estructuras, uniones y bitfields.
/*Fecha de Creación: 17/05/2017
Grupo 4
Integrantes:	Dell'isola Lucas   (58.025)
				Lambertucci Guido  (58.009)
				Tolaba Francisco   (58.424)
Este programa emula los puertos A , B Y D de un microprocesador, teniendo la posibilidad
de modificar los bits de los mismos y aplicarles mascaras. 
*/
//Funcionalidades
#include <stdio.h>
...
//Alias para las utilidades  que tendra el programa
...
int main (void)
{
	...
	int option_validation = TRUE, status;	
	...
	while ( ( option != ENDOFPROGAM ) && ( bit_modify != ENDOFPROGAM ) && ( port != ENDOFPROGAM ))
	{
		...
		if ( (option == BITSET)||(option == BITCLR)||(option == BITGET)||(option == BITTOGGLE) )	// para modificaciones de bits 
		{
			...
			switch (option)
			{
				...
				case BITGET: status = bitget (port, (int) bit_modify, pointer); printf("\nbit %d = %d\n",(int) bit_modify, ( status ? 1 : 0) ); break;
